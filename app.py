...
# --- 2. WEATHER ASSESSMENT (PERMANENTE) ---
...
# --- 3. ADDITIONAL SERVICES (UPDATED LIST) ---
...
def generate_newsletter_html(m_stage, d_icao, d_city, d_fbo, d_time, a_icao, a_city, a_fbo, a_time, d_icon, d_msg, a_icon, a_msg, services):
    ...
    # Dashboard de Servicios (Orden Solicitado)
    svc_data = [
    ...
    for item in svc_data:
        ...
        svc_html += f"""
        <div style="display:inline-block; margin:5px; width:90px; padding:12px 0; border-radius:8px; background:{bg}; border:{border}; text-align:center;">
            <div style="font-size:18px; color:{txt}; font-weight:bold;">◈</div>
            <div style="font-size:8px; color:{txt}; font-weight:bold; margin-top:4px; letter-spacing:0.5px;">{item['label']}</div>
        </div>"""
    svc_html += "</div>"
    ...
    <div style="font-family: Arial, sans-serif; max-width: 550px; border: 2px solid #333; border-radius: 15px; overflow: hidden; margin: auto; background-color: #ffffff;">
        ...
        <div style="padding: 40px; color: #333;">
            ...
            <p style="font-size: 16px; line-height: 1.6; color: #444; text-align: center; margin-bottom: 25px;">{msg_map[m_stage]}</p>
            
            <div style='border-collapse: separate; display: table; width: 100%;'>
                <div style='display: table-cell; width: 48%; padding:20px; background:#fcfcfc; border:1px solid #eee; border-radius:10px; border-top:4px solid {BRAND_COLOR};'>
                    ...
                    <span style='font-size:13px; color:#333; font-weight:500;'>{d_msg if d_msg else "Visual"}</span>
                </div>
                ...
                <div style='display: table-cell; width: 48%; padding:20px; background:#fcfcfc; border:1px solid #eee; border-radius:10px; border-top:4px solid {BRAND_COLOR};'>
                    ...
                    <span style='font-size:13px; color:#333; font-weight:500;'>{a_msg if a_msg else "Visual"}</span>
                </div>
            </div>
...
if st.button("Generate Executive Report"):
    if origin and destination:
        ...
        st.components.v1.html(newsletter, height=950)
    else:
