...
<div style="display: table-cell; width: 48%; padding:20px; background:#fcfcfc; border:1px solid #eee; border-radius:10px; border-top:4px solid {BRAND_COLOR};"><span style="font-size:24px; color:{BRAND_COLOR};">{a_icon}</span><br><b style="font-size:11px; color:#999; text-transform:uppercase;">Arrival WX</b><br><span style="font-size:13px; color:#333; font-weight:500;">{a_msg if a_msg else "Visual"}</span></div></div>"""
    ...
    msg_map = {
        ...
        "FBO Arrival & Boarding Coordination": f"Aircraft is ready at {d_fbo}. The flight crew and FBO staff are standing by to assist with your arrival, check-in, and boarding process. <b>We would appreciate a notification once you are approximately 15 minutes away from the airport so the FBO staff and crew may finalize preparations for your arrival and provide any necessary assistance.<b>",
        "Departure & Enroute Monitoring": "We noticed you are ready to depart, and our team will continue monitoring the flight’s progress through active flight following."
    }
