...
with col1:
    origin = st.text_input("Departure ICAO").upper()
...
with col2:
    destination = st.text_input("Arrival ICAO").upper()
...
if milestone == "Trip Confirmation":
    ...
    st.info("Switch ON items that are already confirmed.")
    c1, c2 = st.columns(2)
...
def generate_newsletter_html(m_stage, d_icao, d_city, d_fbo, d_time, d_tz, a_icao, a_city, a_fbo, a_time, a_tz, sw, d_wx_icon, d_wx_msg, a_wx_icon, a_wx_msg):
    ...
    # --- BRANDING LINE: CHANGE LOGO URL HERE ---
    COMPANY_LOGO_URL = "https://your-company-logo-url.png" # <--- LÍNEA 116: PEGA TU LINK AQUÍ
    ...
    if m_stage == "Trip Confirmation":
        tracker_html = "<div style='margin-top:20px; border-top:1px solid #eee; padding-top:15px;'><b style='font-size:12px; color:#444;'>TRIP PROGRESS:</b><table width='100%' style='margin-top:10px;'>"
        ...
        for cat, active in sw.items():
            color = "#00d4ff" if active else "#cccccc"
    ...
    if m_stage == "Positioning Update":
        wx_display = f"<div style='margin-top:15px; display:flex; gap:10px;'><div style='flex:1; background:#f4faff; padding:10px; border-radius:8px; border-left:4px solid #00d4ff;'><span style='font-size:20px;'>{d_wx_icon}</span><br><b style='font-size:10px; color:#005fcc;'>DEP WX:</b><br><span style='font-size:12px;'>{d_wx_msg}</span></div><div style='flex:1; background:#f4faff; padding:10px; border-radius:8px; border-left:4px solid #00d4ff;'><span style='font-size:20px;'>{a_wx_icon}</span><br><b style='font-size:10px; color:#005fcc;'>ARR WX:</b><br><span style='font-size:12px;'>{a_wx_msg}</span></div></div>"
    ...
    <div style="font-family: Arial, sans-serif; max-width: 500px; border: 1px solid #eee; border-radius: 12px; overflow: hidden; margin: auto; background-color: #ffffff;">
        <div style="background-color: #000; padding: 25px; text-align: center;">
            <img src="{COMPANY_LOGO_URL}" height="50" style="margin-bottom:15px; alt="Company Logo"><br>
            <h2 style="color: #00d4ff; margin: 0; font-size: 16px; letter-spacing: 2px;">{title_map[m_stage]}</h2>
        </div>
        <div style="padding: 25px; color: #333;">
            <div style="text-align: center; margin-bottom: 25px;">
                <div style="font-size: 26px; font-weight: bold;">{d_icao} ✈ {a_icao}</div>
                <div style="font-size: 12px; color: #888;">{d_city} to {a_city}</div>
            </div>
            {tracker_html}
            {wx_display}
            <table width="100%" style="margin-top: 25px; border-top: 1px solid #eee; padding-top: 15px; font-size: 12px;">
                <tr>
                    <td style="width: 50%; vertical-align: top;"><b>DEP:</b> {d_time} ({d_tz})<br><span style="color:#666;">FBO: {d_fbo}</span></td>
                    <td style="width: 50%; vertical-align: top; text-align: right; border-left: 1px solid #eee; padding-left: 10px;"><b>ARR:</b> {a_time} ({a_tz})<br><span style="color:#666;">FBO: {a_fbo}</span></td>
                </tr>
            </table>
        </div>
        <div style="background-color: #000; padding: 10px; text-align: center; font-size: 9px; color: #555;">VIP FLIGHT SUPPORT | OPERATIONAL UPDATE</div>
    </div>
    """
